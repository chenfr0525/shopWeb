...
const good=ref({})
const getGood=async()=>{
  const res=await getGoodDetailService(route.params.id)
...
const addCartHandle=async()=>{
  ...
  cartStore.addCart({
    ...
    count:count.value,
    skuId: skuSelected.value.skuId,
    attrsText:skuSelected.value.specsText,
    selected:true
  })
...
<template>
  <div class="xtx-goods-page" :class="{ loading }" v-loading="loading">
    <div class="container" v-if="good.categories">
      ...
      <div class="info-container">
        <div>
          <div class="goods-info">
            <div class="media">
              <!-- 图片预览区 -->
               <Image :imageList="good.mainPictures"></Image>
              <!-- 统计数量 -->
            ...
            <div class="spec">
              ...
              <!-- sku组件 -->
               <Sku :goods="good" @change="skuChange"></Sku>
              <!-- 数据组件 -->
               <el-input-number v-model="count" @change="countChange"></el-input-number>
              <!-- 按钮组件 -->
              <div>
                <el-button size="large" class="btn" @click="addCartHandle">
                  加入购物车
                </el-button>
              </div>
